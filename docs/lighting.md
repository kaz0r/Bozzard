...
Sun lighting and its existing camera-independent shadows remain available. Spot lights can optionally cast camera-independent local shadows; the `shadows` boolean defaults to `false` and applies only to spots. Point lights retain the shadow settings in the scene format but never cast shadows and do not count toward the local-shadow limit. Up to eight authored shadow-enabled spot components are allowed, including disabled ones; validation rejects additional components. Each active spot uses a fixed 1024px `Depth32Float` layer, while inactive, black, or zero-intensity spots do not allocate or render a layer and an all-inactive set uses a tiny fallback resource. The shadow camera uses a near plane of `min(range * 0.001, 0.05)`, far plane `range`, and FOV `2 * outer_angle`. `shadow_bias` and `shadow_normal_bias` default to `0.005` and `0.01` world units and accept finite values in `0..=1`.

Spot shadows use per-light frustum culling and the same alpha-cutout, sampler/UV/tint-opacity, PBR double-sided, and reflected-winding caster policy as sun shadows. Blended and unlit surfaces do not cast. A spot shadow changes only that spotlight's direct contribution; sun, ambient, emissive, and baked GI remain unaffected, and 2D extraction excludes the feature. The realtime shadow settings are not part of the baked-GI source fingerprint because the CPU bake computes its own visibility.
...
Light markers remain clickable in the 3D viewport. Disabled lights are shown as gray markers and still participate in selection. Selecting a light shows its range guide; selecting a spot also shows its inner and outer cones. Spot inspectors expose a **Cast shadows** checkbox and the bias controls, with a `1024px` / `up to 8` capacity hint. Guides are editor overlays and are hidden in 2D and during Play.
...
5. Render the same scene in 2D and 3D and confirm lights affect only the 3D result. Select enabled and disabled lights to inspect their colored or gray markers, then select a point and spot light to inspect the range and cone guides. Enable **Cast shadows** on a spot and place a caster and receiver inside its cone; confirm the receiver shadows move with the spot and do not change sun, ambient, emissive, or GI lighting. Confirm point lights do not cast shadows, blended/unlit casters are excluded, and 2D extraction has no spot shadows.
6. Add shadow-enabled spots until the eighth is accepted, then confirm the ninth is rejected by validation (disabled spots still count; point lights do not). Toggle a spot inactive, black, or zero intensity and confirm its shadow contribution disappears and the viewport `FrameStats` shadow draw count drops. Adjust both bias controls and confirm values are constrained to finite `0..=1` world units.
...
Native Metal smoke validation passed the existing fixtures plus `local_lights_gpu_ok`, covering PBR and diffuse color response, inverse-square falloff, range cutoff, spot cones and penumbra, equal-angle hard cones, multiple lights, removal, the 32-component limit, invalid inputs, and unlit surfaces. The release Lighting Lab also renders a colored point light and a warm shadow-enabled spotlight for an easy manual check. Its earlier 30-frame 800×500 M2 Pro measurement (0.068 ms optimized renderer CPU median, 0.497 ms synchronized CPU+GPU+wait wall median) predates spotlight shadows and is retained as historical renderer data, not an FPS claim.

Workspace CPU tests, all-target Clippy, and headless boundary checks passed. The earlier local-lights fixtures are included in the existing cross-platform `--smoke` CI coverage. Native light-inspector and guide capture also passed; see the editor validation in the GI section below.
...
Local rendering review covered the release Lighting Lab and a Sponza scene with two spots. At 30 frames and 800×500 on an M2 Pro, the Lighting Lab measured 0.293 ms optimized renderer CPU / 1.172 ms synchronized CPU+GPU+wait wall with shadows on, versus 0.254 ms / 1.014 ms off. Sponza measured 0.701 ms / 2.800 ms on versus 0.728 ms / 2.149 ms off; these are renderer measurements with no CPU speedup claim and are not FPS. Reviewed local ignored captures are `work/spot-shadows-sponza/loaded-3d.png` and `work/spot-shadows-sponza-off/loaded-3d.png`. Per-light culling reduced Sponza shadow work from 206 draws and 524,534 triangles to 93 draws and 277,253 triangles with exact reference/cached pixel parity.
...
Enable **Show volume and probes** to draw the editor-only volume and probe overlay. Cyan indicates a current bake, amber indicates an outdated or unbaked volume, and gray marks invalid probe data. The overlay is disabled in 2D and during Play. Baked indirect visibility does not create realtime local shadow maps; any realtime spot shadows are a separate direct-light feature and are ignored by the GI source fingerprint.
