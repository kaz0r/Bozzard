...
The scene also showcases the [post-processing stack](post-processing.md). The Effects panel exposes each effect and named presets; Live preview animates shimmer, grain, and the [volumetric haze](volumetrics.md). The authored exposure and bloom threshold remain tuned for this dark clearing.
...
Smoke, drifting ash, luminous spark trails, temporal anti-aliasing, motion blur, and wet-stone reflections complete the scene. The Blueprint ember lifecycle remains intact alongside the particle system. `atmosphere-lab.json` offers a wider camera for the combined effects; see [atmosphere and motion effects](atmosphere-effects.md).
