...
Rigidbody remains stored under the existing `gravity` scene key. Existing non-player gravity objects now use real dynamics without conversion. Defaults: acceleration 9.81, max fall speed 50, jump speed 5, mass 1 kg, friction 0.6, restitution 0, linear/angular damping 0/0.1, gravity scale 1. Runtime velocity/grounding reset on spawn. Disabling the component or its collider pauses it and resets velocity. Per-body drag, gravity scale and collision layers/masks are documented in [the physics surface](physics.md).

Dynamic roots support nonuniform and mirrored scale. Parented bodies require a positive, uniformly scaled, non-sheared parent transform; colliding descendants of a Rigidbody now compose into that body as compound shapes, and the editor's WASD box-mover shortcut remains box-only. Dynamic concave decomposition is still not exposed; a dynamic Mesh Collider remains its convex hull. Joints and the capsule Player Controller are documented in [the physics surface](physics.md).
