...
| E press | Interact/use: the Target Range weapon table swaps guns when the player stands at a bay |
...
6. **Camera:** near the tall side wall, look at the wall until it lies between the player and desired camera: the camera pulls inward instead of crossing the collider. Look back: normal distance returns. Leave the viewport (editor) or switch applications (standalone), and return: no stuck orbit/movement. Standalone look is mouse-look while the pointer is captured (no button held), and plain pointer motion when a platform refuses the capture; the pointer capture is released on focus loss, pause, menus and game over.
...
A Rapier capsule character controller: walking, step-up, slope limits, ground snap and moving-platform carry, driven by the authored `capsule_radius`/`capsule_height`/`step_height`/`slope_limit_degrees`. It is not a force-driven character and has no ragdoll; the authored Box Collider still serves the CPU queries. See [the physics surface](physics.md). Trigger overlap is sampled each fixed tick, so very small volumes/extreme movement speeds can be missed. Camera avoidance uses enabled solid boxes with conservative expanded slabs (including transformed/sheared boxes), not render-mesh raycasts; corner cases can pull in early, and a target already inside a solid collapses the boom to the target. Configure adequate clearance for unusually wide aspect ratios/large near planes. Moving obstacles can invalidate a previously safe checkpoint; normal bounded collision recovery still applies at runtime. One player/one active follow camera; no skeletal animation, persistent saved progress, sound or general game export in this milestone.
