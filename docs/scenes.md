...
- `collider`: a local box with `center`, full `size`, `enabled` and the collision `layers`/`mask` bits. A Rigidbody's colliding descendants compose into it as compound shapes. See [the physics surface](physics.md).
- `gravity`: the Rigidbody. `enabled`, positive world-down `acceleration`, `max_speed`, `jump_speed`, `mass`, `friction`, `restitution`, `linear_damping`, `angular_damping` and `gravity_scale`. See [Rigidbody settings](mesh-colliders.md).
- `player_controller`: optional single-player movement/jump/follow-camera settings with a validated active 3D camera ID, plus the character controller's `capsule_radius`, `capsule_height`, `step_height`, `slope_limit_degrees` and `snap_to_ground`. See [the physics surface](physics.md).
- `joint`: one constraint to another object by ID: `kind` (fixed/revolute/spherical/prismatic/rope), local `anchor`/`other_anchor`, local `axis`/`other_axis`, and optional `limits`. See [Joints](physics.md#joints).
- `trigger`: optional local box `volume` plus collectible/checkpoint/goal `action`; separate from solid colliders. See [gameplay format, constraints and runtime-state rules](playable-demo.md#authoring-contract). All additions remain optional in schema v1.
