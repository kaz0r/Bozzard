...
- [ ] **L** Batching/instancing and LOD. Consecutive batching and authored distance LOD are implemented; occlusion culling remains open. Consecutive visible opaque surfaces sharing a mesh and texture now batch into one instanced call of up to 32 objects (uniform-binding bounded), with stock shaders selecting the per-invocation object uniform; skinned geometry, shader graphs, alpha cutoff, text and sprites stay single-draw, order is unchanged, and `set_instancing_enabled(false)` restores the reference path. Measured on an RTX 3060 (Vulkan) at 320×320 with 1,024 lit checker cubes: 1,024 → 32 color draws, CPU 1.32 → 0.87 ms static and 2.77 → 2.29 ms rotating, output byte-identical to the reference path; see [asset rendering](docs/assets.md). LOD supports ordered distance thresholds, imported mesh replacements and explicit far culling, asset dependency/remapping, and inspector editing. A 64×64 RTX 3060/Vulkan check measures 12 → 1 → 0 color triangles for cube → imported OBJ → culled, matching explicitly authored reference pixels; see [LOD](docs/lod.md). Automatic mesh simplification and occlusion culling remain open.
