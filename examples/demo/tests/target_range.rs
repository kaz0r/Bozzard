//! End-to-end checks for the Target Range scene: a Blueprint player controller, first-person
//! aiming, pointer capture and the shoot-all-four win.
...
/// The controller graph turns `yaw - mouse_x * sensitivity` degrees; authoring is the scene's job.
const SENSITIVITY: f32 = 0.2;
const EYE_HEIGHT: f32 = 0.5;

fn load() -> (Scene, PathBuf) {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scenes/target-range.json");
...
#[test]
fn a_blueprint_controller_moves_aims_jumps_and_respawns_the_player() {
    let (scene, _) = load();
    let player = scene.objects.iter().find(|o| o.id == "player").unwrap();
    assert!(
        player.player_controller.is_none() && player.gravity.is_none(),
        "the scene must drive the player from graphs, not the built-in controller"
    );
    let mut demo = demo();
    // The graph's own gravity settles the body on the platform.
    ...
    assert!(
        respawned,
        "past the edge nothing holds the body up: the graph's own gravity takes over"
    );
...
fn the_scene_locks_the_cursor_draws_a_crosshair_and_hands_it_back_on_a_win() {
    ...
    assert_eq!(
        ...
        Some(true),
        "the player graph must claim the pointer for mouse-look"
    );
    ...
    assert_eq!(
        demo.app
            ...
            .and_then(|capture| capture.requested),
        Some(false),
        "the win graph must release the pointer before the run ends"
    );
