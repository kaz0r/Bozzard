...
for slot in range(4):
    ...
    if slot:
        objects.extend([b, k])
bird['steam_multiplayer'] = {'game': 'flap_woods', 'protocol': 1, 'app_id': 480, 'max_players': 4}
for obj in objects:
...
for obj in objects:
    if obj['id'].startswith('steam-friend-') or obj['id'] in ('steam-friends-next', 'steam-friends-back'):
