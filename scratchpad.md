...
Commit the upload scheduling checkpoint after final native Sponza validation. Then implement authored lighting and shadows, followed by environment/IBL, exposure/tone mapping, and performance counters with measured culling/draw improvements. All remain required for the active goal; fixed preview illumination is not final lighting. Do not claim completion until actual Sponza visibly looks good and relevant native checks pass.
...
- Renderer stages model geometry/textures across frames and shares vertex buffers/images within a model. ModelUploadStats reports memory, surfaces/images, preparation and observed upload slices (not GPU time). Opaque then center-sorted transparent passes; no culling/shadows/IBL yet.
...
Latest full test log: `/tmp/bozzard-upload-tests.log`. Durable docs: README, docs/assets.md, docs/playable-demo.md, docs/roadmap.md. Earlier detailed handoff in `424098e:scratchpad.md`; older debugging history in `bc22cc8:scratchpad.md`.
