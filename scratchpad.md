...
Commit authored lighting when final GPU validation passes. Implement sun shadow maps next, including alpha-cutout casters, appropriate light bounds/bias/filtering, and pixel evidence that changing light direction moves shadows while emissive/ambient stay unshadowed. Then environment/IBL, exposure/tone mapping, and performance counters with measured culling/draw improvements. All remain required for the active goal. Do not claim completion until actual Sponza visibly looks good and relevant native checks pass. Point/spot lights are not present; current directional lighting is scene-wide rather than an entity component.
...
- Renderer stages model geometry/textures across frames and shares vertex buffers/images within a model. ModelUploadStats reports memory, surfaces/images, preparation and observed upload slices (not GPU time). Opaque then center-sorted transparent passes; authored sun/ambient, no culling/shadows/IBL yet.
...
Latest full test log: `/tmp/bozzard-lighting-tests.log`. Durable docs: README, docs/assets.md, docs/playable-demo.md, docs/roadmap.md. Earlier detailed handoff in `424098e:scratchpad.md`; older debugging history in `bc22cc8:scratchpad.md`.
