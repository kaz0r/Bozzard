...
After committing residency, implement the actual byte-budgeted GPU upload queue across frames in the shared bridge and both application lifecycles, preserving transactional replacement/cancellation. Residency is preparation, not completion of upload scheduling. Do not substitute one-whole-asset-per-frame: Sponza is one large asset. Split texture row writes and geometry/attribute/index buffers into bounded byte chunks, schedule mip work, and atomically install only completed assets. Keep old GPU data usable while replacements stage; discard stale/cancelled jobs. Immutable source ownership must remain alive for all image identity keys (a renderer-facing source trait implemented by the bridge can retain Arc CPU data without making renderer depend on assets). Expose progress, and verify intermediate frames plus replacement/failure/cancellation with GPU fixtures. Startup/CLI may drain the same queue synchronously while editor and hot reload advance it each frame. Then add authored lights/shadows/environment/exposure/tone mapping, and measured culling/performance improvements. All remain required; current fixed preview illumination is not final lighting.
...
Latest full test log: `/tmp/bozzard-residency-tests.log`. Durable docs: README, docs/assets.md, docs/playable-demo.md, docs/roadmap.md. Earlier detailed handoff in `424098e:scratchpad.md`; older debugging history in `bc22cc8:scratchpad.md`.
