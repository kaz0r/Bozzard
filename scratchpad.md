...
## Current checkpoint — point-light shadows (validated)

Implemented on user-requested feature branch `codex/point-light-shadows`, from pushed `ac33742`. This checkpoint commits the validated subsystem per the standing instruction. **No push requested.** Root owns implementation/tests/scratchpad; GPT-5.6 Luna (`point_shadow_docs`) updated README and lighting/Sponza documentation. Preserve Bozz tribute. No mouse movement.
...
- Full workspace CPU tests, all-target denied-warning Clippy, headless dependency audit, formatting and diff checks passed. Final debug and release native Metal suites passed. Native editor acceptance passed authoring/controls/GI bake/save and existing workflows. Fresh capture workspace `work/editor-point-shadows-complete/` avoids restoring the expanded GI inspector from earlier captures. No hosted CI claimed; branch has not been pushed. Context7 tools unavailable; used pinned wgpu30.0.1 and official WGSL reference.
...
Manual test: `cargo run -p bozzard-editor-app -- --scene examples/sponza/point-lights.json`, select **Warm point** or **Cool point**, toggle **Cast shadows**. Asset-free alternative: `examples/demo/scenes/lighting-lab.json`. Await user review/push request.
...
Point-light shadows are locally validated and committed on `codex/point-light-shadows`; await user testing and an explicit push request. Hosted Metal/Vulkan/DX12 CI should be verified after that push. A possible next subsystem is better baked-GI probe visibility around thin walls; await user direction. Path tracing remains a later renderer project.
