...
The Hierarchy search filters object names and IDs without case sensitivity, including nested objects. Clear it with **×** to restore the full tree. Select an object and press **⌘Return** on Mac / **Ctrl+Enter** elsewhere, **F2**, or click **Rename** to edit its name inline in Edit mode: **Enter** applies one undoable change, while **Escape** or clicking away cancels. Starting a rename clears the search so the selected row is visible. Right-click a Hierarchy row for **Rename**, **Duplicate** (⌘D / Ctrl+D), **Frame Selection** (⌘Shift+F / Ctrl+Shift+F), and **Delete** (⌘Backspace on Mac, or Delete). Menu actions target the clicked row and show platform-specific primary shortcuts: Mac shows Cmd+Return, Cmd+D, Cmd+Shift+F and Cmd+Backspace; Windows/Linux show F2, Ctrl+D, Ctrl+Shift+F and Delete. Alternate rename bindings remain available. Shortcuts are inactive while typing; these actions are Edit-only. The existing F / Shift+F viewport framing shortcuts remain available. Double-click an object in the Hierarchy (including search results) to select and frame it and its children in the current viewport layer while in Edit mode; this does not change the authored camera or undo history. During Play, the Gravity inspector displays **Rising**, **Falling**, or **Grounded** and signed vertical speed.
