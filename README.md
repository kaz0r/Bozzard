...
Environment colors, intensity, background, and reset are available in the Scene lighting controls. On first enabled use, the GPU builds a 32px diffuse cosine-convolved cubemap, a 128px GGX specular cubemap with 8 mip levels, a 64px split-sum BRDF LUT, and 128 deterministic Hammersley samples; later color/intensity edits update uniforms only. The environment is a single procedural distant sky, not an HDR panorama, reflection-probe, GI, or atmospheric system.
